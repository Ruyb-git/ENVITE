...
const eventSchema = z.object({
  ...
  banners_upload: z
    .custom<FileList>()
    .optional()
    .refine((files) => !files || files.length <= 5, 'Maximum of 5 images allowed')
    .refine(
...
const LocationPicker: React.FC<LocationPickerProps> = ({ position, onPositionChange, mapRef }) => {
...
const EventForm: React.FC<EventFormProps> = ({
  ...
  const [mapPosition, setMapPosition] = useState<[number, number]>([
    ...
    initialData?.longitude || -41.467270,
  ]);
  ...
  const { control, handleSubmit, formState: { errors }, setValue } = useForm({
    ...
    defaultValues: {
      ...
      longitude: initialData?.longitude || -41.467270,
    },
  });
  ...
  const handleAddressSearch = async (e: React.FormEvent | React.KeyboardEvent) => {
    e.preventDefault();
        ...
        render={({ field: { onChange, value, ...field } }) => (
          <div>
            ...
            {errors.banners_upload && (
              <p className="mt-1 text-sm text-red-600">{errors.banners_upload.message}</p>
            )}
          </div>
        )}
      />
